...
def read_i(f, n=1, forceList=False):  # type: (any, int, bool) -> Union[int, Tuple[Any, ...]]
...
def read_s(f, length=None):  # type: (any, int) -> str
    return f.read(read_i(f) if length is None else length).decode("utf-8", errors="ignore")


def read_bytes(f, n=1):  # type: (any, int) -> list[int]
    return list(f.read(n))


def write_i(f, values):  # type: (any, Union[list[int], int]) -> bool
    if not isinstance(values, (list, tuple)):
        values = [values]
    return f.write(struct.pack('I' * len(values), *values))


def write_s(f, value):  # type: (any, str) -> bool
    write_i(f, len(value))
    return f.write(bytearray(value.encode()))


def write_bytes(f, value):  # type: (any, bytes) -> bool
...
class RefractorFlatArchiveInfo:
    def __init__(self, f=None, csize=None, ucsize=None, doffset=None):
        self.csize = read_i(f) if f is not None else csize
        self.ucsize = read_i(f) if f is not None else ucsize
        self.doffset = read_i(f) if f is not None else doffset
...
class RefractorFlatArchive:
        ...
        try:
            with open(self.path, 'rb') as f:
                ...
                if self.fileSize >= 28:
                    ...
                    isV1dot1 = read_s(f, 28) in ["Refractor2 FlatArchive 1.1  "]
                    if not isV1dot1:
                        f.seek(0)
                offset = read_i(f)
    ...
    def extractBlock(self, file_info, destinationPath=None, asBytes=False):  # type: (RefractorFlatArchiveInfo, Union[str, None], bool) -> Union[bool, bytes]
        self.success = False
        try:
            with open(self.path, 'rb') as f:
                data = []
                ...
                if data:
                    if destinationPath is None:
                        ...
                        for data_segment in data:
                            ret_str += data_segment if asBytes else data_segment.decode("utf-8", errors="ignore")
                        return ret_str
                    directory_path = os.path.dirname(destinationPath)
    ...
    def extractFile(self, path, destinationDir=None, asString=False):
        ...
        destinationPath = path if destinationDir is None else os.path.join(destinationDir, path)
        for file in self.fileList:
            if file.path == path:
    ...
    def addFileAsString(self, relativePath, contents):
        ...
        self.fileList.append(
            RefractorFlatArchiveEntry(relativePath, is_external=True, is_string=True, file_contents=contents))

    def addDirectory(self, directory, base_directory=None):
        if base_directory is None: base_directory = directory
    ...
    def deleteAllNonServerFiles(self):
        for i in reversed(range(len(self.fileList))):
            ...
            if os.path.splitext(filePath)[1].lower() in ['.bik', '.dds', '.tga', 'wav'] or os.path.basename(
                    filePath).lower() in ['palette.pal', 'envmap_g_.rcm', 'lightmapshadowbits.lsb',
                                          'terrainpalette.pal', 'textureprecache.dat']:
                self.fileList.pop(i)

    def write(self, destPath=None, compressed=True):
        overWriteSelf = destPath is None
        if destPath is None:
            destPath = self.path + "tmp"
        ...
        with open(destPath, "wb") as f:
            ...
            randomBytesSring = "Refractor Flat Archive Packed with Arkylien's Python Module on " + datetime.now().strftime(
                "%Y-%m-%d %H:%M:%S")
            ...
            write_i(f, (self.xpackHeaderId if self.xpackHeaderId != None else 0x48128321) + sum(
                randomBytes))  # xpackHeaderId
            ...
            for file in self.fileList:
                ...
                if not compressed:  # not compressed
                    ...
                    fileBytesBlocks = [fileBytes[i:min(i + maxSegmentSize, len(fileBytes))] for i in
                                       range(0, len(fileBytes), maxSegmentSize)]
                    ...
                    for fileBytesBlock in fileBytesBlocks:
                        ...
                        segmentInfos.append(
                            RefractorFlatArchiveInfo(None, len(fileBytesCompressed), len(fileBytesBlock),
                                                     f.tell() - startDataBlocks))
                ...
                file_infos.append((file.path, RefractorFlatArchiveInfo(None, csize, len(fileBytes), dataOffset)))
...
class RefractorFlatArchiveGroup:
    def __init__(self, rfas=None):
        self.rfas = [] if rfas is None else [RefractorFlatArchive(path) for path in rfas]

    def extractFile(self, path, destinationDir=None, asString=False):
        for rfa in self.rfas:
            filePathInRFA = rfa.getCorrectFilePath(path)
            if filePathInRFA is not None:
